...
- **Resolution flow**
  - Example: `Ad Spec + Surface Profile → Constraint Resolver → Resolved Layout → Renderer`
- **Limitations**
...
- Demo the same spec resolving across all required surfaces
- Introduce a new, previously-unseen surface profile live and resolve it
- Explain the priority/degradation algorithm step by step
- Walk through why a specific element ended up at a specific position/size for a given surface
- Discuss how this could extend to broadcast-safe-area or print-bleed constraints
